...
void B2PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if ( targetLV ) targetR = dynamic_cast<G4Sphere*>(targetLV->GetSolid())->GetOuterRadius();
  ...
  G4double radius = targetR; // 半径为target半径
  G4double r = radius * sqrt(G4UniformRand()); // 均匀分布在圆内
  G4double theta = 2 * CLHEP::pi * G4UniformRand();

  G4cout << "Range of Generator Gun is " << radius/um << " um radius" << G4endl;
